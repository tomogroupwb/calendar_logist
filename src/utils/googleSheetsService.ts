...
export const fetchDeliveryDataFromConfig = async (configId: string): Promise<DeliveryEvent[]> => {
  ...
  if (accessToken) {
    ...
    if (isTokenExpired()) {
      ...
      console.warn('[WARN] Логика обновления токена требует пересмотра после удаления Firebase.');
      // Пока что просто очистим токен и потребуем повторной авторизации
      clearToken();
      throw new Error('Требуется повторная авторизация OAuth, так как токен истек, а Firebase удален.');
    }
  }
  ...
  if (!config) {
    console.error(`[ERROR] Конфигурация с ID ${configId} не найдена в localStorage.`);
    // Если конфигурация не найдена, возвращаем пустой массив или выбрасываем ошибку
    // В данном случае, возвращаем пустой массив, чтобы не прерывать работу приложения критически
    // но это поведение может потребовать уточнения в зависимости от бизнес-логики.
    return [];
  }
  ...
  try {
    ...
    console.error(`[ERROR] Ошибка при получении/обработке данных из Google Sheets:`, error);
...
export const verifyAndRefreshToken = async (): Promise<boolean> => {
  ...
  if (!accessToken || !refreshToken) {
    console.log('Отсутствует access token или refresh token. Пропускаем проверку/обновление.');
    return false; // Не можем обновить без токенов
  }

  if (isTokenExpired()) {
    console.log('Токен недействителен, пробуем обновить...');
    // TODO: Реализовать обновление токена Google напрямую, если это требуется
    // Сейчас эта логика закомментирована, так как refreshFirebaseGoogleToken удален
    // const refreshSuccess = await refreshGoogleToken(); // Hypothetical function
    // if (refreshSuccess) {
    // console.log('Токен успешно обновлен');
    // return true;
    // } else {
    // console.error('Не удалось обновить токен. Требуется повторная авторизация.');
    // clearToken(); // Очищаем токены, так как обновление не удалось
    // return false;
    // }
    console.warn('[WARN] Логика обновления токена Google требует реализации без Firebase.');
    clearToken(); // Очищаем токены, так как обновление не удалось и Firebase удален
    return false;
  }
...
/**
 * Обновление данных в Google Sheets с использованием конкретной конфигурации
 * @param updatedEvent - Событие с обновленными данными
 * @param configId - ID конфигурации (необязательно, если есть только одна)
 */
  ...
  try {
    ...
    if (!actualConfigId) {
      ...
      if (configs.length === 1) {
        ...
        console.error('Несколько конфигураций найдено, необходимо указать configId');
        throw new Error('Необходимо указать configId для обновления данных при нескольких конфигурациях.');
      } else {
        console.error('Нет доступных конфигураций для обновления данных.');
        throw new Error('Нет доступных конфигураций для обновления данных.');
      }
    }
    ...
    if (!config) {
      throw new Error(`Конфигурация с ID ${actualConfigId} не найдена в localStorage`);
    }

    // Продолжаем с localStorage конфигурацией
